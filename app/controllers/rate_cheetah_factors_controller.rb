class RateCheetahFactorsController < ApplicationController
  ...
  def show
    ...
    total_factors = current_user.rateable_cheetah_factors.count
    current_index = current_user.rateable_cheetah_factors.to_a.index(@cheetah_factor) + 1
    @percent_complete = [(current_index.to_f / total_factors) * 100, 99].min
  end
  ...
  def index
    ...
    next_factor = current_user.rateable_cheetah_factors.first
    ...
    if career
      @previous = user_career_rate_cheetah_factor_path(career, current_user.rateable_cheetah_factors.last, section_id: params[:section_id], repeat: params[:repeat])
  ...
  def next_path
    if next_factor = current_user.rateable_cheetah_factors.where(["row_order > ?", @cheetah_factor.row_order]).first
  ...
  def previous_path
    if previous_factor = current_user.rateable_cheetah_factors.where(["row_order < ?", @cheetah_factor.row_order]).last
